	...
	int main()
	{
		...
		do 
		{
			...
			case 9: copyArray(a, b, n);
				...
				if (duration > 0)
				{
					cout << "Thoi gian chay tim kiem tuan tu la:  " << duration * 1000000 << " miliseconds" << endl;
				}
				break;
			case 10:  copyArray(a, b, n);
				start = clock();
				cout << "Nhap vao x muon tim:  \n";
				cin >> x;
				 tam=Search(b, n,x);
				duration = (clock() - start) / (double)CLOCKS_PER_SEC;
				if (tam ==-1)
				{
					cout << "Khong tim thay phan tu x="<< x<<"trong danh sach\n";
				}
				else {
					cout << "Da tim thay phan tu x=" << x << "trong danh sach\n";
				}
				if (duration > 0)
				{
					cout << "Thoi gian chay tim kiem nhi phan la:  " << duration * 1000000 << " miliseconds" << endl;
				}
				break;
			case 11:  copyArray(a, b, n);
				start = clock();
				cout << "Nhap vao x muon tim:  \n";
				cin >> x;
				tam = BinarySearch(b, n, x);
				duration = (clock() - start) / (double)CLOCKS_PER_SEC;
				if (tam == -1)
				{
					cout << "Khong tim thay phan tu x=" << x << "trong danh sach\n";
